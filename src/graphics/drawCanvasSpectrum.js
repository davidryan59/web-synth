...
import { getSliderOutputValue } from '../getters/slider'
...
const drawCanvasSpectrum = (objStore, reduxStore) => {
  ...
  const minVal = getSliderOutputValue(reduxStore.getState(), ui.ANALYSER_MIN_DB)
  const maxVal = 0
  const canvasCtx = objStore.ctx.canvas.spectrum
  const canvasElt = objStore.elt.canvas.spectrum
  const maxDisplayFreq = getSliderOutputValue(reduxStore.getState(), ui.ANALYSER_MAX_FREQ)
